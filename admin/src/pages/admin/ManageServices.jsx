...
export const ManageServices = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingId) {
      const updated = await adminService.updateService(editingId, form);
      if (updated) {
        setServices(prev => prev.map(s => (s._id === editingId || s.id === editingId) ? updated : s));
        setIsAdding(false);
        setForm({ title: "", description: "", icon: "🚀" });
        setEditingId(null);
      }
  ...
  return (
    <div>
      ...
      <Modal isOpen={isAdding} onClose={() => setIsAdding(false)} title={editingId ? "Edit Service" : "Add Service"}>
      ...
      <div style={{ 
        ...
        {services.map(service => (
          <motion.div key={service._id || service.id} whileHover={{ y: -4 }}
            style={{ 
            ...
            <div style={{ position: "absolute", top: "16px", right: "16px", display: "flex", gap: "8px" }}>
              ...
              <button onClick={() => handleDelete(service._id || service.id)}
                style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", transition: "0.2s" }}
